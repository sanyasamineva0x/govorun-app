...
extension TextMode {
    ...
    static func basePrompt(currentDate: Date, personalDictionary: [String: String] = [:]) -> String {
        ...
        ЧИСЛА И ДАТЫ:
        - Числительные → цифры: «три ночи» → «3 ночи», «пять кг» → «5 кг»
        - Исключение: «одного/одну/одной/двух» → словом: «двух дизайнеров»
        - Порядковые в датах → цифры: «первого апреля» → «1 апреля»
        - Деньги → символ (₽, $), пробелы в тысячах: «20 000 ₽»
        - Время → HH:MM только при явном времени суток или точном формате:
          «в восемь вечера» → «в 20:00», «в пять утра» → «в 05:00», «в пятнадцать тридцать» → «в 15:30»
        - Если времени суток нет, не додумывай: «завтра в пять» → «Завтра в 5»
        - Проценты → %. Русская локаль: запятая в дробях, пробелы в тысячах
        - «номер one two three four» → «№1234»
        ...
        «завтра в пять» → «Завтра в 5»
        «ой точнее пете» → «Пете»
        «ну давай завтра после обеда созвонимся по бюджету» → \
        «Давай завтра после обеда созвонимся по бюджету»
        «позвони маме в восемь вечера или нет лучше в девять» → «Позвони маме в 21:00»
        ...
        «Напиши, что дедлайн не пятница, а понедельник»
        «напиши в телеграм что я опоздаю минут на пятнадцать» → \
        «Напиши в Telegram, что я опоздаю минут на 15»
