...
Замеры на MacBook Air M1, 8 GB RAM:

| Метрика | Значение | Условия |
|---------|----------|---------|
| **Cold start** | 0.9 сек | Загрузка модели + VAD в память |
| **Warm inference** | 300–360 мс | ~5 сек аудио |
| **Тишина** | 17 мс | Пустая запись |
| **Длинная запись** | ~1.2 сек | ~60 сек аудио (VAD нарезка) |
| **WER** | 6.9% | GigaAM-v3 e2e_rnnt, русский |
| **RAM (worker)** | ~1.5 GB | GigaAM + Silero VAD в памяти |
| **RAM (Swift app)** | ~50 MB | Без учёта worker |
| **CPU (idle)** | 0% | Worker спит между запросами |
| **CPU (inference)** | 100–200% | 1–2 ядра на время распознавания |
| **App bundle** | 67 MB | С embedded Python.framework |
| **Модель на диске** | 892 MB | Скачивается один раз |
